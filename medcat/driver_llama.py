...
config.general.model_name = "meta-llama/Meta-Llama-3-8B"
config.train.nclasses = 9
config.model.model_size= 2304
config.model.hidden_size= 256
config.train.batch_size = 64
config.train.nepochs = 4
config.model.model_size = 12288

config.general.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
model_config = AutoConfig.from_pretrained("meta-llama/Meta-Llama-3-8B",token='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',num_hidden_layers=5,pad_token_id=128260,vocab_size=128265)
print(model_config)
...
peft_config = LoraConfig(task_type=TaskType.SEQ_CLS, inference_mode=False, r=8, lora_alpha=16,
                          target_modules=["q_proj", "v_proj"], lora_dropout=0.2)
#
